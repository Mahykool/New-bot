...
const roleFor = async (jid) => {
  ...
  const base = { 
    ...
    isMods: global.mods.includes(num), // ← agregado
    isAdmin: false, 
    isBotAdmin: false 
  }
...
const badgeFor = async (jid) => {
  ...
  if (r.isMods) b.push('MOD') // ← agregado
  if (r.isAdmin) b.push('ADMIN')
  if (r.isPrems) b.push('PREMIUM')
  if (global.botNums.includes(normalizeCore(jid))) b.push('BOT')
  return b
}
...
if (plugin.mods && !rolesCtx.isMods) { fail('mods', m, this); continue } // ← corregido
...
// =======================
// Mensajes de error (dfail) cortos y cómicos con ctx activos
// =======================
global.dfail = (type, m, conn, usedPrefix) => {
  ...
  const cfg = {
    rowner:   { text: '🤣 Solo el jefe supremo puede usar esto.', ctx: ctxDenied },
    owner:    { text: '🙃 Solo el creador, no tú.', ctx: ctxDenied },
    mods:     { text: '🕵️ Solo moderadores, agente secreto fallido.', ctx: ctxDev },
    premium:  { text: '💎 Solo VIP, tú sigues en modo demo.', ctx: ctxDenied },
    group:    { text: '👥 Solo en grupos, no en solitario.', ctx: ctxInfo },
    private:  { text: '💌 Solo en privado tupidx.', ctx: ctxInfo },
    admin:    { text: '🛡️ Necesitas admin, no basta con carisma.', ctx: ctxDenied },
    botAdmin: { text: '🤖 El bot debe ser admin, no mago.', ctx: ctxInfo },
    unreg:    { text: '📋 Regístrate, fantasma sin identidad.', ctx: ctxInfo },
    restrict: { text: '🚫 Esta función está deshabilitada, intenta otra cosa.', ctx: ctxInfo },
  }[type]
