...
// ⬇️ Aquí agregas la función fail
function fail(reason, m, conn) {
  let msg = ''
  switch (reason) {
    case 'mods': msg = '❌ Este comando es solo para moderadores/owner.'; break
    case 'premium': msg = '✨ Este comando es solo para usuarios premium.'; break
    case 'group': msg = '👥 Este comando solo funciona en grupos.'; break
    case 'botAdmin': msg = '🤖 El bot necesita ser admin para usar este comando.'; break
    case 'admin': msg = '🛡️ Este comando es solo para administradores.'; break
    case 'private': msg = '📩 Este comando solo funciona en chats privados.'; break
    case 'unreg': msg = '⚠️ Necesitas registrarte para usar este comando.'; break
    default: msg = '❌ No tienes permiso para usar este comando.'
  }
  if (conn && m) {
    conn.reply(m.chat, msg, m)
    conn.sendMessage(m.chat, { react: { text: '✖', key: m.key } }) // opcional
  }
}
