...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    conn.sendMessage = async function(jid, content, options = {}) {
        ...
        if (global.rcanal && global.rcanal.contextInfo) {
            ...
            newContent.contextInfo = {
                ...existingContext,
                ...global.rcanal.contextInfo,
            };
        }

        return originalSendMessage.call(this, jid, newContent, options);
    };
    ...
    const sock = Object.defineProperties(conn, {
        ...
        decodeJid: {
            value(jid) {
                ...
                try {
                    // usa jidDecode importado de Baileys si está disponible
                    if (typeof jidDecode === 'function') {
                        const dec = jidDecode(jid)
                        if (dec && dec.user && dec.server) return `${dec.user}@${dec.server}`
                    }
                    // fallback: quitar la parte de dispositivo si existe (ej: 12345:1@...)
