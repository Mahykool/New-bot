...
// Middleware de permisos SW SYSTEM — versión mejorada y compatible con ESM

import fs from 'fs'
import path from 'path'
import { getRolesConfig, getUserRoles, normalizeJid } from './lib-roles.js'
...
function loadLevels() {
  ...
  try {
    ...
    const data = safeReadJson(file, { basic: 1, manage: 5, full: 10 })
    CACHE.levels = data
    CACHE.levelsMtime = stat
    return data
  } catch (e) {
    return { basic: 1, manage: 5, full: 10 }
  }
}
...
// Puedes ajustar el mapeo aquí si tu jerarquía difiere
...
// Devuelve la "clave" de nivel del usuario basada en su rol principal
export function getUserLevel(userJid) {
  ...
  // Si es string y existe en levels, devolverla
  ...
  // Si es número, mapear a la clave correspondiente
...
export function canUseCommand(userJid, pluginId, command) {
  ...
  // Owner override: si es owner global, permitir todo
  try {
    if (global.owner) {
      const owners = Array.isArray(global.owner) ? global.owner.flat() : [global.owner]
      for (const o of owners) {
  ...
  // Si pluginPermissions define el comando, usarlo
  ...
  // Si no hay entrada específica, fallback a política por defecto: require 'manage'
  const defaultRequired = 'manage'
  const userLevelKey = getUserLevel(normalized)
  const userPower = levels[userLevelKey] ?? 0
  const requiredPower = levels[defaultRequired] ?? 5
  return userPower >= requiredPower
}
