...
export const toNum = jid => String(jid || '').replace(/@s\.whatsapp\.net/g, '').replace(/\D/g, '')
export const jidFull = num => (num && num.includes('@')) ? num : `${toNum(num)}@s.whatsapp.net`
export const isValidNum = n => /^\d{6,15}$/.test(String(n || '').replace(/\D/g, ''))
...
// Export default con utilidades principales
const libRoles = {
  toNum,
  jidFull,
  isValidNum,
