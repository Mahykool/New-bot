// lib/lib-roles.js
// Sistema de roles centralizado para SW SYSTEM — versión final (actualizado con auditoría)
...
export function getLevelsConfig() {
  // formato esperado: { basic: 1, vip: 2, vip_plus: 3, manage: 4, full: 5 } o similar
  return loadJson(LEVELS_PATH, { basic: 1, vip: 2, vip_plus: 3, manage: 4, full: 5 })
}
...
export function setUserRole(userJid, roleId, actor = null) {
  ...
  // Auditoría: registrar la acción aunque el guardado falle (se intenta siempre)
  try {
    auditLog({ actor: actor || null, target: jid, action: 'SET_ROLE', role: normalizeRoleId(roleId), ok: !!ok })
  } catch (e) {
...
export function loadRoles() {
  // Devuelve la configuración actual de roles (lee desde roles.json)
  return getRolesConfig()
}

export function reloadRoles() {
  // Alias que fuerza recarga y devuelve la configuración actual
  return loadRoles()
}
