  ...
  try {
    ...
    try {
      if (typeof conn.profilePictureUrl === 'function') {
        avatarUrl = await conn.profilePictureUrl(participant, 'image')
      }
    } catch (err) {
      avatarUrl = ''
    }
    ...
    // Enviar como imagen con caption y mentions (más compatible)
    const caption = [
      '╭─ SW SYSTEM ─╮',
      `│ Usuario: ${taguser}`,
      `│ Grupo: ${groupSubject}`,
    ...
    // Intentar enviar con la firma moderna; si falla, fallback a sendFile
    try {
      if (typeof conn.sendMessage === 'function') {
        await conn.sendMessage(jid, { image: fs.readFileSync(file), caption, mentions }, { quoted: null })
      } else if (typeof conn.sendFile === 'function') {
        await conn.sendFile(jid, file, path.basename(file), caption, { mentions })
      } else {
        // último recurso: sendMessage con product (si tu cliente lo acepta)
